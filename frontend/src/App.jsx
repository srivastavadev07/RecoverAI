...
function App() {
  ...
  async function executeRecovery() {
    ...
    if (
      ...
      setExecutionError(
        "Invalid recovery action selected by AI."
      );
  ...
  const urgentOpportunities =
    summary?.urgent_opportunities ?? 0;


  const revenueRecovered =
    evaluation?.revenue_recovered ?? 0;


  const recoveryAttempts =
    evaluation?.recovery_attempts ?? 0;


  const successfulRecoveries =
    evaluation?.successful_recoveries ?? 0;


  const blockedActions =
    evaluation?.blocked_actions ?? 0;


  const recoveryRate =
    (evaluation?.recovery_rate ?? 0) * 100;
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <MetricCard
              ...
              value={`₹${expectedRecovery.toLocaleString(
                "en-IN"
              )}`}
          ...
          {!selectedPayment && (
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                RecoverAI will analyze the payment and recommend
